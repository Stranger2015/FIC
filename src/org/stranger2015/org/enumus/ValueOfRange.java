...
public
class ValueOfRange<T extends Enum <T>, F /*extends Comparable <ValueOfRange <T, F>>*/> {
    private static final String ERROR_MESSAGE = "No enum constant %s.%s";
    private final Class <T> type;
    private final NavigableMap <F, T> values;

    public
    ValueOfRange ( Class <T> type, Function <T, F> floorAccessor, Function <T, F> ceilingAccessor ) {
        this.type = type;
        values = new TreeMap ();
    ...
    private static
    <F, T> void fill ( Map <F, T> map, T[] values, Function <T, F> keyCreator ) {
