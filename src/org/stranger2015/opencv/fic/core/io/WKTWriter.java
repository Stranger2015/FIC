...
import static org.locationtech.jts.io.WKTConstants.*;
import static org.stranger2015.opencv.fic.core.io.OrdinateFormat.DEFAULT;
import static org.stranger2015.opencv.fic.core.io.OrdinateFormat.create;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.EMPTY;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.M;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.POINT;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.POLYGON;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.Z;
import static org.stranger2015.opencv.fic.core.io.WKTConstants.*;
...
class WKTWriter {
     ...
     */
    public static
    String toPoint ( Coordinate p0 ) {
        return String.format("%s ( %s )", POINT, format(p0));
    }
     ...
     */
    public static
    String toLineString ( Coordinate[] coord ) {
     ...
     */
    public static
    String toLineString ( Coordinate p0, Coordinate p1 ) {
        return LINE_STRING + " ( " + format(p0) + ", " + format(p1) + " )";
    }

    public static
    String format ( Coordinate p ) {
        return format(p.x, p.y);
    }

    private static
    String format ( double x, double y ) {
        return DEFAULT.format(x) + " " + DEFAULT.format(y);
    }
     ...
     */
    private static
    OrdinateFormat createFormatter ( PrecisionModel precisionModel ) {
        return create(precisionModel.getMaximumSignificantDigits());
    }
     ...
     */
    private static
    String stringOfChar ( char ch, int count ) {
        return IntStream.range(0, count).mapToObj(i -> valueOf(ch)).collect(Collectors.joining());
    }
    ...
    class CheckOrdinatesFilter implements CoordinateSequenceFilter {
        ...
        /**
         * @see org.stranger2015.opencv.fic.core.geom.CoordinateSequenceFilter#isDone
         */
        public
        boolean isDone () {
     ...
     */
    public
    void setOutputOrdinates ( EnumSet <Ordinate> outputOrdinates ) {
    ...
    void setPrecisionModel ( PrecisionModel precisionModel ) {
        ...
        this.ordinateFormat = create(precisionModel.getMaximumSignificantDigits());
    }
     ...
     */
    private
    void writeFormatted ( Geometry geometry, boolean useFormatting, Writer writer )
            throws IOException {
    ...
    private
    OrdinateFormat getFormatter ( Geometry geometry ) {
     ...
     */
    private
    void appendGeometryTaggedText ( Geometry geometry, boolean useFormatting, Writer writer, OrdinateFormat formatter )
            throws IOException {
        // evaluate the ordinates actually present in the geometry
        WKTWriter.CheckOrdinatesFilter cof = new CheckOrdinatesFilter(this.outputOrdinates);
        geometry.apply(cof);
    ...
    void appendGeometryTaggedText (
        ...
        if (geometry instanceof Point) {
            ...
            Assert.shouldNeverReachHere(String.format("Unsupported Geometry implementation:%s", geometry.getClass()));
        }
    }
    ...
    void appendGeometryCollectionTaggedText (
        ...
        writer.write(GEOMETRY_COLLECTION);
        writer.write(" ");
        appendOrdinateText(outputOrdinates, writer);
     ...
     */
    private static
    String writeNumber ( double d, OrdinateFormat formatter ) {
     ...
     */
    private
    void appendOrdinateText ( EnumSet <Ordinate> outputOrdinates, Writer writer ) throws IOException {
        if (outputOrdinates.contains(Ordinate.Z)) {
            writer.append(Z);
        }
        if (outputOrdinates.contains(Ordinate.M)) {
     ...
     */
    private
    void appendMultiLineStringText ( @NotNull MultiLineString multiLineString,
                                     EnumSet <Ordinate> outputOrdinates,
                                     boolean useFormatting,
                                     int level,
            /*boolean indentFirst, */Writer writer,
                                     OrdinateFormat formatter )
            throws IOException {
    ...
    void appendMultiPolygonText (
            @NotNull MultiPolygon multiPolygon,
            EnumSet <Ordinate> outputOrdinates,
            boolean useFormatting,
            int level,
            Writer writer,
    ...
    private
    void indentCoords ( boolean useFormatting, int coordIndex, int level, Writer writer )
            throws IOException {
    ...
    private
    void indent ( boolean useFormatting, int level, Writer writer )
            throws IOException {
