...
class QuadEdgeSubdivision {
     ...
     */
    public static
    void getTriangleEdges ( QuadEdge startQE, QuadEdge[] triEdge ) {
        triEdge[0] = startQE;
    ...
    void createFrame ( Envelope env ) {
        ...
        double offset = deltaX > deltaY ? deltaX * 10.0 : deltaY * 10.0;
    ...
    QuadEdge locate ( Coordinate p0, Coordinate p1 ) {
        ...
        QuadEdge base = e.dest().getCoordinate().equals2D(p0) ? e.sym() : e;
     ...
     */
    public
    List <QuadEdge> getPrimaryEdges ( boolean includeFrame ) {
        visitedKey++;

        List <QuadEdge> edges = new ArrayList <>();
        Deque <QuadEdge> edgeStack = new ArrayDeque <>();
...
//    }
     ...
     */
    public
    List <QuadEdge[]> getTriangleEdges ( boolean includeFrame ) {
    ...
    class TriangleEdgesListVisitor implements TriangleVisitor {
        ...
        public
        List <QuadEdge[]> getTriangleEdges () {
            return triList;
        }
    }
     ...
     */
    public
    List <Vertex[]> getTriangleVertices ( boolean includeFrame ) {
    ...
    class TriangleVertexListVisitor implements TriangleVisitor {
        ...
        public
        List <Vertex[]> getTriangleVertices () {
            return triList;
        }
    }
     ...
     */
    public
    List <Coordinate[]> getTriangleCoordinates ( boolean includeFrame ) {
    ...
    class TriangleCoordinatesVisitor implements TriangleVisitor {
        ...
        protected final CoordinateList coordList = new CoordinateList();
        protected final List <Coordinate[]> triCoords = new ArrayList <>();
        ...
        private
        void checkTriangleSize ( Coordinate[] pts ) {
            String loc = "";
            if (pts.length >= 2) {
                loc = org.stranger2015.opencv.fic.core.io.WKTWriter.toLineString(pts[0], pts[1]);
            }
            else {
                if (pts.length >= 1) {
    ...
    Geometry getEdges ( GeometryFactory geomFact ) {
        List <QuadEdge> quadEdges = getPrimaryEdges(false);
 ...
 new Triangle<>();
     ...
     */
    public
    List <Coordinate[]> getVoronoiCellPolygons ( GeometryFactory geomFact ) {
        /*
        ...
        Collection <QuadEdge> edges = getVertexUniqueEdges(false);
     ...
     */
    public
    Coordinate[] getVoronoiCellPolygon ( QuadEdge qe, GeometryFactory geomFact ) {
        List <Coordinate> cellPts = new ArrayList <>();
        QuadEdge startQE = qe;
        do {
//    	Coordinate cc = circumcentre(qe);
        ...
        return cellPoly.getCoordinates();//fixme
    }
