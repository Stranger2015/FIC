...
import static org.stranger2015.opencv.fic.core.math.DD.sqr;
import static org.stranger2015.opencv.fic.core.math.DD.valueOf;
...
class TrianglePredicate {
     ...
     */
    public static
    boolean isInCircleNonRobust ( Coordinate a, Coordinate b, Coordinate c, Coordinate p ) {
     ...
     */
    public static
    boolean isInCircleNormalized ( Coordinate a, Coordinate b, Coordinate c, Coordinate p ) {
        double adx = a.x - p.x;
        double ady = a.y - p.y;
        double bdx = b.x - p.x;
        double bdy = b.y - p.y;
     ...
     */
    private static
    double triArea ( Coordinate a, Coordinate b, Coordinate c ) {
    ...
    @Contract(pure = true)
    public static
    boolean isInCircleRobust ( Coordinate a, Coordinate b, Coordinate c, Coordinate p ) {
...
//    return isInCircleNonRobust(a, b, c, p);
     ...
     */
    public static
    boolean isInCircleDDSlow ( Coordinate a, Coordinate b, Coordinate c, Coordinate p ) {
        DD px = valueOf(p.x);
        DD py = valueOf(p.y);
        DD ax = valueOf(a.x);
        DD ay = valueOf(a.y);
     ...
     */
    public static
    DD triAreaDDSlow ( DD ax, DD ay, DD bx, DD by, DD cx, DD cy ) {
        return (bx.subtract(ax).multiply(cy.subtract(ay)).subtract(by.subtract(ay)
                .multiply(cx.subtract(ax))));
    }

    public static
    boolean isInCircleDDFast ( Coordinate a, Coordinate b, Coordinate c,Coordinate p ) {
        DD aTerm = sqr(a.x).selfAdd(sqr(a.y))
                .selfMultiply(triAreaDDFast(b, c, p));
        DD bTerm = sqr(b.x).selfAdd(sqr(b.y))
                .selfMultiply(triAreaDDFast(a, c, p));
        DD cTerm = sqr(c.x).selfAdd(sqr(c.y))
                .selfMultiply(triAreaDDFast(a, b, p));
        DD pTerm = sqr(p.x).selfAdd(sqr(p.y))
    ...
    DD triAreaDDFast (
        ...
        DD t1 = valueOf(b.x).selfSubtract(a.x)
                .selfMultiply(
                        valueOf(c.y).selfSubtract(a.y));

        DD t2 = valueOf(b.y).selfSubtract(a.y)
                .selfMultiply(
                        valueOf(c.x).selfSubtract(a.x));

        return t1.selfSubtract(t2);
    }

    public static
    boolean isInCircleDDNormalized (
            Coordinate a, Coordinate b, Coordinate c,
            Coordinate p ) {
        DD adx = valueOf(a.x).selfSubtract(p.x);
        DD ady = valueOf(a.y).selfSubtract(p.y);
        DD bdx = valueOf(b.x).selfSubtract(p.x);
        DD bdy = valueOf(b.y).selfSubtract(p.y);
        DD cdx = valueOf(c.x).selfSubtract(p.x);
        DD cdy = valueOf(c.y).selfSubtract(p.y);
     ...
     */
    private static
    void checkRobustInCircle ( Coordinate a, Coordinate b, Coordinate c,
                               Coordinate p ) {
