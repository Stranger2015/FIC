...
class FacetSequence {
    ...
    private ICoordinateSequence pts;
    private final int start;
    private final int end;

    /**
     * Creates a new sequence of facets based on a {@link ICoordinateSequence}
     ...
     */
    public
    FacetSequence ( Geometry geom, ICoordinateSequence pts, int start, int end ) {
        this.geom = geom;
        this.pts = pts;
        this.start = start;
        this.end = end;
     ...
     */
    public
    FacetSequence ( ICoordinateSequence pts, int start, int end ) {
        this.pts = pts;
        this.start = start;
        this.end = end;
    }
     ...
     */
    public
    FacetSequence ( ICoordinateSequence pts, int start ) {
        this.pts = pts;
        this.start = start;
        this.end = start + 1;
    }
    ...
    GeometryLocation[] nearestLocations ( FacetSequence facetSeq ) {
        ...
        if (isPoint && isPointOther) {
            ...
            org.locationtech.jts.operation.distance.GeometryLocation tmp = locs[0];
            locs[0] = locs[1];
            locs[1] = tmp;
        }
        else {
    ...
    private
    double computeDistanceLineLine ( org.locationtech.jts.operation.distance.FacetSequence facetSeq, org.locationtech.jts.operation.distance.GeometryLocation[] locs ) {
    ...
    private
    void updateNearestLocationsLineLine ( int i,
                                          Coordinate p0,
                                          Coordinate p1,
    ...
    private
    double computeDistancePointLine ( Coordinate pt, FacetSequence facetSeq, GeometryLocation[] locs ) {
        double minDistance = Double.MAX_VALUE;
    ...
    private
    void updateNearestLocationsPointLine ( Coordinate pt,
                                           FacetSequence facetSeq,
                                           int i,
                                           Coordinate q0,
                                           Coordinate q1,
