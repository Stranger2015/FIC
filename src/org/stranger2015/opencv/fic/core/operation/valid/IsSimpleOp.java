...
class IsValidOp {
    ...
    boolean isValid ( Coordinate coord ) {
        return
                !Double.isNaN(coord.x) &&
                !Double.isInfinite(coord.x) &&
                !Double.isNaN(coord.y) &&
                !Double.isInfinite(coord.y);
    }
    ...
    private
    boolean hasInvalidError () {
        return validErr != null;

    }

    private
    boolean isValidGeometry ( Geometry g ) {
        validErr = null;
        ...
        if (g instanceof LineString) {
            return isValid((LineString) g);
        }
        if (g instanceof Polygon) {
            return isValid((Polygon) g);
        }
        if (g instanceof MultiPolygon) {
            return isValid((MultiPolygon) g);
        }
        if (g instanceof GeometryCollection) {
            return isValid((GeometryCollection) g);
        }
    ...
    boolean isValid ( Polygon g ) {
        ...
        var areaAnalyzer = new PolygonTopologyAnalyzer(g, isInvertedRingValid);
     ...
     */
    private
    boolean isValid ( MultiPolygon g ) {
     ...
     */
    private
    boolean isValid ( GeometryCollection gc ) {
        return IntStream.range(0, gc.getNumGeometries())
                .allMatch(i -> isValidGeometry(gc.getGeometryN(i)));
    }

    private
    void checkCoordinateInvalid ( Coordinate[] coords ) {
        IntStream.range(0, coords.length).filter(i -> !isValid(coords[i]))
                .findFirst().ifPresent(i -> logInvalid(TopologyValidationError.INVALID_COORDINATE,
                        coords[i]));
    }

    private
    void checkCoordinateInvalid ( Polygon poly ) {
    ...
    private
    void checkRingNotClosed ( LinearRing ring ) {
        if (ring.isEmpty()) {
            return;
        }
        if (!ring.isClosed()) {
    ...
    private
    void checkRingsNotClosed ( Polygon poly ) {
     ...
     */
    private
    void checkTooFewPoints ( LineString line, int minSize ) {
     ...
     */
    private
    boolean isNonRepeatedSizeAtLeast ( LineString line, int minSize ) {
        int numPts = 0;
     ...
     */
    private
    void checkSelfIntersectingRing ( LinearRing ring ) {
     ...
     */
    private
    void checkShellsNested ( MultiPolygon mp ) {
    ...
    private
    void checkInteriorDisconnected ( PolygonTopologyAnalyzer analyzer ) {
