...
import org.stranger2015.opencv.fic.core.math.MathUtil;
...
class CoordinateArrays {
     ...
     */
    public static
    void enforceConsistency ( Coordinate[] array ) {
        if (array == null) {
            return;
        }
        // step one check
     ...
     */
    public static
    Coordinate[] enforceConsistency ( Coordinate[] array, int dimension, int measures ) {
        Coordinate sample = Coordinates.create(dimension, measures);
        Class <?> type = sample.getClass();
        boolean isConsistent = Arrays.stream(array)
                .noneMatch(
                        coordinate -> coordinate != null &&
                                !coordinate.getClass().equals(type)
                );
        if (isConsistent) {
            return array;
        }
            ...
            IntStream.range(0, copy.length).forEachOrdered(i -> {
                Coordinate coordinate = array[i];
                if (coordinate != null && !coordinate.getClass().equals(type)) {
     ...
     */
    public static
    Coordinate ptNotInList ( Coordinate[] testPts, Coordinate[] pts ) {
        return Arrays.stream(testPts).filter(testPt -> indexOf(testPt, pts) < 0)
     ...
     */
    public static
    int compare ( Coordinate[] pts1, Coordinate[] pts2 ) {
        int i = 0;
     ...
     */
    public static
    int increasingDirection ( Coordinate[] pts ) {
     ...
     */
    private static
    boolean isEqualReversed ( Coordinate[] pts1, Coordinate[] pts2 ) {
...
//    }
         ...
         */
        public static
        Coordinate[] copyDeep ( Coordinate[] coordinates ) {
         ...
         */
        public static
        void copyDeep ( Coordinate[] src, int srcStart, Coordinate[] dest, int destStart, int length ) {
            IntStream.range(0, length)
                    .forEachOrdered(i -> dest[destStart + i] = src[srcStart + i].copy());
        }
         ...
         */
        public static
        Coordinate[] toCoordinateArray ( Collection <Coordinate> coordList ) {
         ...
         */
        public static
        boolean hasRepeatedPoints ( Coordinate[] coord ) {
            return IntStream.range(1, coord.length)
                    .anyMatch(i -> coord[i - 1].equals(coord[i]));
        }
         ...
         */
        public static
        Coordinate[] atLeastNCoordinatesOrNothing ( int n, Coordinate[] c ) {
         ...
         */
        public static
        Coordinate[] removeRepeatedPoints ( Coordinate[] coord ) {
         ...
         */
        public static
        boolean hasRepeatedOrInvalidPoints ( Coordinate[] coord ) {
         ...
         */
        public static
        Coordinate[] removeRepeatedOrInvalidPoints ( Coordinate[] coord ) {
         ...
         */
        public static
        Coordinate[] removeNull ( Coordinate[] coord ) {
            int nonNull = 0;
            for (int i = 0; i < coord.length; i++) {
        ...
        boolean equals (
            ...
            return IntStream.range(0, coord1.length).allMatch(i -> coord1[i].equals(coord2[i]));
        }
        ...
        boolean equals (
                Coordinate[] coord1,
                Coordinate[] coord2,
                Comparator <org.stranger2015.opencv.fic.core.geom.Coordinate> coordinateComparator ) {
            if (coord1 == coord2) {
                return true;
            }
            ...
            return coord1.length == coord2.length && IntStream.range(0, coord1.length)
                    .noneMatch(i -> {
                        return coordinateComparator.compare(coord1[i], coord2[i]) != 0;
                    });
         ...
         */
        public static
        Coordinate minCoordinate ( Coordinate[] coordinates ) {
         ...
         */
        public static
        void scroll ( Coordinate[] coordinates, Coordinate firstCoordinate ) {
         ...
         */
        public static
        void scroll ( Coordinate[] coordinates, int indexOfFirstCoordinate ) {
         ...
         */
        public static
        void scroll ( Coordinate[] coordinates, int indexOfFirstCoordinate, boolean ensureRing ) {
            ...
            if (!ensureRing) {
                System.arraycopy(coordinates, indexOfFirstCoordinate, newCoordinates, 0, coordinates.length - indexOfFirstCoordinate);
                System.arraycopy(coordinates, 0, newCoordinates, coordinates.length - indexOfFirstCoordinate, indexOfFirstCoordinate);
            }
            else {
         ...
         */
        public static
        int indexOf ( Coordinate coordinate, Coordinate[] coordinates ) {
         ...
         */
        public static
        Coordinate[] extract ( Coordinate[] pts, int start, int end ) {
         ...
         */
        public static
        Envelope envelope ( Coordinate[] coordinates ) {
            Envelope env = new Envelope();
            Arrays.stream(coordinates).forEachOrdered(env::expandToInclude);
         ...
         */
        public static
        Coordinate[] intersection ( Coordinate[] coordinates, Envelope env ) {
            CoordinateList coordList = new CoordinateList();
            IntStream.range(0, coordinates.length)
                    .filter(i -> env.intersects(coordinates[i]))
                    .forEachOrdered(i -> coordList.add(coordinates[i], true));
