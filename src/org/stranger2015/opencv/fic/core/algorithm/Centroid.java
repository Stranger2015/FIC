...
class Centroid {
     ...
     */
    public static
    Coordinate getCentroid ( Geometry geom ) {
    ...
    private
    void addShell ( Coordinate[] pts ) {
        if (pts.length > 0)
            setAreaBasePoint(pts[0]);
        boolean isPositiveArea = !org.stranger2015.opencv.fic.core.algorithm.Orientation.isCCW(pts);
    ...
    private
    void addTriangle ( Coordinate p0, Coordinate p1, Coordinate p2, boolean isPositiveArea ) {
     ...
     */
    private static
    void centroid3 ( Coordinate p1, Coordinate p2, Coordinate p3, Coordinate c ) {
        c.x = p1.x + p2.x + p3.x;
        c.y = p1.y + p2.y + p3.y;
        return;
    }
     ...
     */
    private static
    double area2 ( Coordinate p1, Coordinate p2, Coordinate p3 ) {
        return
                (p2.x - p1.x) * (p3.y - p1.y) -
                        (p3.x - p1.x) * (p2.y - p1.y);
    }
     ...
     */
    private
    void addLineSegments ( Coordinate[] pts ) {
        double lineLen = 0.0;
        for (int i = 0; i < pts.length - 1; i++) {
