...
public class LargestEmptyCircle {
    ...
    private IndexedPointInAreaLocator pLocator;
    ...
    private void setBoundary(Geometry obstacles) {
        ...
        if (boundary.getDimension() >= 2) {
            pLocator = new IndexedPointInAreaLocator(boundary);
    ...
    private double distanceToConstraints(Point p) {
        boolean isOutide = Location.EXTERIOR == pLocator.locate(p.getCoordinate());
        if (isOutide) {
    ...
    private boolean mayContainCircleCenter( Cell cell) {
         ...
         */
        if (cell.isOutside()) {
            return cell.getMaxDistance() > tolerance;
        }
    ...
    private
    Cell createCentroidCell( Geometry geom) {
