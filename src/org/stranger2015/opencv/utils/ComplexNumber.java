...
class ComplexNumber {
    ...
    public
    String toString () {
        String re = String.format("%s", this.real);
        String im = this.imaginary < 0 ?
                String.format("%si", this.imaginary) :
                String.format("+%si", this.imaginary);
     ...
     *
     * @param formatId the format ID <code>ComplexNumber.XY</code> or <code>ComplexNumber.RCIS</code>.
     * @return a string representation of the complex number
     * @throws IllegalArgumentException if the formatId does not match.
     */
    public
    String format ( int formatId ) throws IllegalArgumentException {
        String out;
        switch (formatId) {
            case XY:
                out = toString();
                break;
            case RCIS:
                out = String.format("%s cis(%s)", mod(), getArg());
                break;
            default:
